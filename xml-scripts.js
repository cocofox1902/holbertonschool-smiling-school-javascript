window.onload = function () {
  ...
  function quotes() {
    const url = "https://smileschool-api.hbtn.info/xml/quotes";
    ...
    $.ajax({
      ...
      success: (result) => {
        let active = "active";
        result.forEach((quote, i) => {
          active = i === 0 ? "active" : "";
          $("#car-inner-1").append(`
              <div class="carousel-item ${active}">
                <div class="container">
                  <div class="d-flex flex-column flex-md-row align-items-center text-white">
                    <img class="quote-pic rounded-circle" src=${quote.pic_url} alt="avatar" width="200px">
                    <div class="ml-md-5 mr-md-0 flex-column">
                      <p style="font-size: 22px;">« ${quote.text}</p>
                      <p class="pt-2 mb-0" style="font-size: 22px;"><b>${quote.name}</b></p>
                      <p><i style="font-size: 18px;">${quote.title}</i></p>
                    </div>
                  </div>
                </div>
              </div>
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        for (let i = 0; i < result["topics"].length; i++) {
          topics += `<option value=${
            result["topics"][i].charAt(0).toUpperCase() +
            result["topics"][i].slice(1)
          }>
                  ${
                    result["topics"][i].charAt(0).toUpperCase() +
                    result["topics"][i].slice(1)
        ...
        for (let j = 0; j < result["sorts"].length; j++) {
          sorts += `<option value=${
            result["sorts"][j].charAt(0).toUpperCase() +
            result["sorts"][j].slice(1)
          }>
                  ${(
                    result["sorts"][j].charAt(0).toUpperCase() +
                    result["sorts"][j].slice(1)
        ...
        for (let i = 0; i < result["courses"].length; i++) {
          if (topic == "All") {
            ...
            if (result["courses"][i].topic == topic) {
              if (keywords) {
                if (
                  result["courses"][i].keywords.includes(
                ...
                console.log(
                  "published at = " + result["courses"][i].published_at
                );
